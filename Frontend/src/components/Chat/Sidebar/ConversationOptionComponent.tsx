...
const ConversationOptionComponent: React.FC<ConversationOptionComponentProps> = ({ option, selected, selectConversation }) => {
    return (
        <li className="grid grid-cols-[1fr_15px] h-6 overflow-hidden px-0.5">
            <button
                className={`text-xs text-left hover:underline hover:bg-zinc-400 rounded-sm px-0.5 ${selected && "bg-zinc-400"} whitespace-nowrap overflow-hidden text-ellipsis`}
            ...
            <button
                aria-label="options"
                className="grid grid-cols-1 grid-rows-3 rounded-sm text-xs ml-0.5 py-1.5 hover:bg-zinc-400">
